...
import pickle
import os
import argparse # To easily pass the PDB ID from the command line
...
def inspect_single_protein(pdb_id_to_inspect):
    ...
    STRUCTURED_DATA_PATH = 'output/structured_protein_data.pkl'
    PREDICTION_THRESHOLD = 0.6 # Confidence score to be considered a predicted epitope

    # --- 1. Load Model and Data ---
    if not os.path.exists(MODEL_PATH) or not os.path.exists(STRUCTURED_DATA_PATH):
    ...
    # Recreate the exact same train/test split to find which proteins are in the test set
    all_pdb_ids = [p['pdb_id'] for p in protein_data_list]
    unique_groups = np.unique(all_pdb_ids)
    _, test_groups = train_test_split(unique_groups, test_size=0.2, random_state=42)
    test_groups = set(test_groups) # Use a set for fast lookups
    ...
    print(f"Number of PREDICTED epitope residues (score >= {PREDICTION_THRESHOLD}): {len(predicted_epitopes)}")
