...
from sklearn.metrics import roc_auc_score, average_precision_score, confusion_matrix, ConfusionMatrixDisplay, precision_recall_curve
...
def main():
    MODEL_PATH = 'models/final_model.json' # Using the final model is standard practice
    ...
    save_precision_recall_curve(y_test, y_test_pred_proba, os.path.join(OUTPUT_DIR, 'pr_curve.png'))
    
    y_pred_class = (y_test_pred_proba > 0.5).astype(int)
    save_confusion_matrix(y_test, y_pred_class, os.path.join(OUTPUT_DIR, 'confusion_matrix.png'))
